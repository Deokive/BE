...
public class GalleryController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "잘못된 페이지 요청 (범위 초과)",
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "403", description = "등록 권한 없음 (아카이브 소유자가 아님) 또는 파일 접근 권한 없음",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                            schema = @Schema(implementation = ErrorResponse.class),
                            examples = @ExampleObject(value = "{\"status\": \"FORBIDDEN\", \"error\": \"AUTH_FORBIDDEN\", \"message\": \"접근 권한이 없습니다.\"}"))),
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "403", description = "삭제 권한 없음 (아카이브 소유자가 아님)",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                            schema = @Schema(implementation = ErrorResponse.class),
                            examples = @ExampleObject(value = "{\"status\": \"FORBIDDEN\", \"error\": \"AUTH_FORBIDDEN\", \"message\": \"접근 권한이 없습니다.\"}"))),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 아카이브입니다.",
